...
class ABTest {
    ...
    getUid() {
        return this.uid || cookieParse(document.cookie)[ABTEST_UID_COOKIE_NAME + this.app_id]
    }
    getTraceId() {
        return this.trace_id || cookieParse(document.cookie)[ABTEST_TRACE_ID_COOKIE_NAME + this.app_id]
    }
    run(layer_id) {
        let uid = this.getUid() || uuid();
        let hash_id = `${uid}_${location.protocol + '://' + location.host + location.path}`;
